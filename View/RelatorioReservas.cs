...
namespace View
{
    ...
    public class RelatorioReservas : Form
    {
        public RelatorioReservas()
        {
            ...
            doc.Save(System.IO.Directory.GetCurrentDirectory() + @"\RelatorioReservas.pdf");
            doc.Close(true);

            var p = new Process();
            p.StartInfo = new ProcessStartInfo(System.IO.Directory.GetCurrentDirectory() + @"\RelatorioReservas.pdf")
            {
